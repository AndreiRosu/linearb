...
describe('generic filter - intersection', () => {
  const files = ['filters.test.ts', 'controller.spec.ts', 'hello.js'];

  it('Should return the common element', () => {
    const result = GENERAL_FILTERS_HANDLER[GeneralFilters.intersection](files, {
      list: ['filters.ts', 'filters.test.ts'],
    });
    expect(result).toEqual(['filters.test.ts']);
  });

  it('Should return an empty list', () => {
    const result = GENERAL_FILTERS_HANDLER[GeneralFilters.intersection](files, {
      list: ['filters.ts', 'filters.test1.ts'],
    });
    expect(result).toEqual([]);
  });

  it('Should return an empty list when intersecting an empty list', () => {
    const result = GENERAL_FILTERS_HANDLER[GeneralFilters.intersection](files, {
      list: [],
    });
    expect(result).toEqual([]);
...
describe('generic filter - difference', () => {
  const files = ['filters.test.ts', 'controller.spec.ts', 'hello.js'];

  it('Should return the uncommon elements from the first list', () => {
    const result = GENERAL_FILTERS_HANDLER[GeneralFilters.difference](files, {
      list: ['filters.ts', 'filters.test.ts'],
    });
    expect(result).toEqual(['controller.spec.ts', 'hello.js']);
  });

  it('Should return the entire first list', () => {
    const result = GENERAL_FILTERS_HANDLER[GeneralFilters.difference](files, {
      list: ['filters.ts', 'filters.test1.ts'],
    });
  ...
  it('Should return the entire first list when intersecting an empty list', () => {
    const result = GENERAL_FILTERS_HANDLER[GeneralFilters.difference](files, {
      list: [],
    });
    expect(result).toEqual(files);
  ...
  it('Should return an empty list', () => {
    const result = GENERAL_FILTERS_HANDLER[GeneralFilters.difference]([], {
      list: files,
    });
    expect(result).toEqual([]);
  });
